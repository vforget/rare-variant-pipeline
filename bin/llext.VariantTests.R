...
suppressPackageStartupMessages(library(getopt, lib.loc="/usr/local/Rlib"))
#suppressPackageStartupMessages(library(Rsge))
suppressPackageStartupMessages(library(Rsge, lib.loc="/usr/local/Rlib"))
suppressPackageStartupMessages(library(doMC, lib.loc="/usr/local/Rlib"))
...
# for debugging
#opt = list(indFile = "ft4.2Dec2010.txt", snpFile="AAAS.polyphen2.weight",
#           genoFile="AAAS.chr12.qc.geno", flipPhenotype=0,
#           permutations=10, nodes=2, profile=1, multicore=0, seed=1)

if (is.null(opt$permutations)) { opt$permutations = 10 }
...
ind <- read.table(opt$indFile, col.names=c("indid", "pheno"), as.is=c(T, T))
csnp <- read.table(opt$snpFile, col.names=c("snpid", "polyphen"), as.is=c(T, T))
cgeno <- read.table(opt$genoFile, col.names=c("indid", "snpid", "count"), as.is=c(T, T, T))
...
# Items needed for CG's implementation of Liu & Leal
   csnp$rare01 = (csnp$counts/(2*N)<0.01)    # 1% threshold for rare
                                           # note this is NOT right since 
                                           # N is # phenotyped not genotyped
   csnp$rare05 = (csnp$counts/(2*N)<0.05)
ind$norare01 = sapply(ind$indid,  function(x)  {  all(is.na(match(m1[m1$indid==x,]$snpid, csnp$snpid[csnp$rare01] )))
      })
ind$norare05 = sapply(ind$indid,  function(x)  {
        all(is.na(match(m1[m1$indid==x,]$snpid, csnp$snpid[csnp$rare05] )))})
   # list of IDs excluding those with rare variants
...
getScores <- function(permute){
  ...
  # new code by CG for Liu & Lean extension
  ...
  negstat.b.01 <- -stat.b  # do this because score expected to get larger under permuted null.  Fit into existing machinery
  pheno.mean <- mean(pheno.cg[ind$norare05])
  pheno.sd <- sd(pheno.cg[ind$norare05])
  quant <- pnorm(pheno.cg, pheno.mean, pheno.sd)
  probnonnull <- 1 - 2*(quant*(quant<0.5) + (1-quant)*(quant>=0.5))
  stat.b <- t(abs(pheno.cg-meanpheno)) %*% probnonnull / sum(probnonnull)
  negstat.b.05 <- -stat.b  # do this because score expected to get larger under permuted null.  Fit into existing machinery

  c(score1=score1, score1P=score1P, score2=score2, score2P=score2P, score3=score3, score3P=score3P, score4=score4, 
    score4P=score4P,  negstat.b.01 = negstat.b.01, negstat.b.05 = negstat.b.05)
}
...
getPermWins <- function(subrange, seed){
  ...
  pw.nll01 <- permwins(scores.df, "negstat.b.01")
  pw.nll05 <- permwins(scores.df, "negstat.b.05")
  c(score1=pw1, score1P=pw1P, score2=pw2, score2P=pw2P, score3=pw3, 
    score3P=pw3P, score4=pw4, score4P=pw4P, negstat.b.01=pw.nll01,
    negstat.b.05=pw.nll05)
}
